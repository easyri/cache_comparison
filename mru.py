...
class MRUCache(LRUCache):
    """
    The Most Recently Used (MRU) cache is like :class:`.Cache` and :class:`.LRUCache`
    but uses a most-recently-used replacement policy.
    ...
    The primary difference with :class:`.LRUCache` is that cache entries are evicted
    from the end of the eviction queue first instead of evicting from the beginning.
    """
